(function ($) {
  ...
  $(function () {
    ...
    function mp_open_modal() {
      if (!$(".os-modal").length) {
        $("body").append(
          '<div class="os-modal-w"><div class="os-modal no-padding"></div><div class="os-meal-planner-fader"></div></div>'
        );
      }
    ...
    function mp_close_modal() {
      ...
      if ($("#mealPlannerForm").data("changed") === "yes") {
        confirm_close = confirm(
          "Are you sure you want to close meal planner? Changes you made will not be saved."
        );
      }
    ...
    function mp_init_nutritions_chart($day) {
      ...
      var label_protein = $(".meal-planner-w .mpd-legend .legend-protein").data(
        "label"
      );
      var label_carbs = $(".meal-planner-w .mpd-legend .legend-carbs").data(
        "label"
      );
      var label_fat = $(".meal-planner-w .mpd-legend .legend-fat").data(
        "label"
      );
      ...
      var total_nutritions =
        nutritions_protein + nutritions_carbs + nutritions_fat;
      var nutritions_protein_percent =
        total_nutritions > 0
          ? Math.round((nutritions_protein / total_nutritions) * 100)
          : 0;
      var nutritions_carbs_percent =
        total_nutritions > 0
          ? Math.round((nutritions_carbs / total_nutritions) * 100)
          : 0;
      var nutritions_fat_percent =
        total_nutritions > 0
          ? Math.round((nutritions_fat / total_nutritions) * 100)
          : 0;
      ...
      nutritionsChartObj["day_" + position] = new Chartist.Pie(
        "#" + day_id + " .mpd-chart",
        data,
        {
          showLabel: false,
    ...
    function mp_recalculate_day_nutritions($day) {
      ...
      if (nutritionsChartObj["day_" + position] !== undefined) {
        ...
        nutritionsChartObj["day_" + position].data.series[1].value =
          total_carbs;
        nutritionsChartObj["day_" + position].data.series[2].value =
          total_protein;
        nutritionsChartObj["day_" + position].update();
      }
      $day
        .find(".mpd-legend-label.legend-carbs span")
        .text(total_carbs + nutritionsMeasure);
      $day
        .find(".mpd-legend-label.legend-fat span")
        .text(total_fat + nutritionsMeasure);
      $day
        .find(".mpd-legend-label.legend-protein span")
        .text(total_protein + nutritionsMeasure);
    }
    ...
    function init_meal_planner() {
      ...
      $("#mp_days_total").on("change", function () {
        ...
        if (current_days_total > new_days_total) {
          var $days_to_remove = $(".meal-planner-w .mp-day").slice(
            new_days_total
          );
          var remove_confirm = true;
          if ($days_to_remove.find(".mpdp-recipe").length) {
            remove_confirm = confirm(
              "Some of the days that you want to remove have recipes, are you sure you want to remove them?"
            );
          }
        ...
        if (current_days_total < new_days_total) {
          for (
            var day_position = current_days_total + 1;
            day_position <= new_days_total;
            day_position++
          ) {
            var $cloned_day = $(".meal-planner-w .mp-day:last")
              .clone()
              .data("position", day_position)
              .data("calories", 0)
              .data("protein", 0)
              .data("carbs", 0)
              .data("fat", 0);
            ...
            $cloned_day
              .find(".mpd-legend .mpd-legend-label span")
              .text("0" + nutritionsMeasure);
            mp_init_nutritions_chart($(".meal-planner-w .mp-day:last"));
            $(".meal-planner-w .mp-day:last .mpd-body").perfectScrollbar();
            dragulaObj.containers = $(
              ".mpd-period-recipes, .recipe-holder"
            ).toArray();
      ...
      $(".select_meal_plan_btn, .create_new_meal_plan_btn").on(
        "click",
        function () {
      ...
      mp_init_drag(
        $(".meal-planner-w .mpd-period-recipes, .recipe-holder").toArray()
      );
    ...
    $(".print-meal-plan").on("click", function () {
      ...
      var wait_label = $(".meal-plans-list").length
        ? $(".meal-plans-list").data("wait-label")
        : $(this).data("wait-label");
      $print_btn
        .data("temp-label", $print_btn.text())
        .find("span")
        .text(wait_label);
      $.ajax({
        type: "post",
    ...
    $(".delete-meal-plan").on("click", function () {
      ...
      $(this)
        .data("temp-label", $(this).text())
        .text($(".meal-plans-list").data("wait-label"));
