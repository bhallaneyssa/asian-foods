function userpro_bm_dialog(elem, html, position) {
  ...
  if (html == "new_collection") {
    elem.append(
      '<div class="userpro-bm-dialog bm-' +
        position +
        '"></div><div class="userpro-bm-dialog-icon bm-' +
        position +
        '"><i class="userpro-icon-caret-up"></i></div>'
    );
    elem
      .find(".userpro-bm-dialog")
      .width(elem.parents(".userpro-bm").width() - 42);
    custom_html =
      '<form action="" method="post"><input type="text" name="userpro_bm_new" id="userpro_bm_new" value="" class="userpro-bm-input" placeholder="' +
      elem.parents(".userpro-bm").data("new_collection_placeholder") +
      '" /><br> <input type="radio" class="userpro_bm_radio" name="public" value="private" checked=checked/>Private <br> <input type="radio" name="public" value="public" class="userpro_bm_radio" />Public <div class="userpro-bm-btn-contain bm-block"><a href="#" class="userpro-bm-btn" data-action="submit_collection">' +
      elem.parents(".userpro-bm").data("add_new_collection") +
      "</a></div></form>";
  } else {
    elem.append(
      '<div class="userpro-bm-dialog bm-' +
        position +
        ' autoclose"></div><div class="userpro-bm-dialog-icon bm-' +
        position +
        ' autoclose"><i class="userpro-icon-caret-up"></i></div>'
    );
    elem
      .find(".userpro-bm-dialog")
      .width(elem.parents(".userpro-bm").width() - 42);
  ...
  elem
    .find(".userpro-bm-dialog")
    .html('<span class="userpro-bm-dialog-content">' + custom_html + "</span>");

  if (jQuery("#userpro_bm_new").length) jQuery("#userpro_bm_new").focus();

  var timer = setTimeout(function () {
    jQuery(".userpro-bm-dialog.autoclose,.userpro-bm-dialog-icon.autoclose")
      .hide()
      .remove();
  }, 3000);
}
...
function userpro_bm_donebookmark_category(elem, html) {
  elem
    .addClass("bookmarked_category")
    .removeClass("unbookmarked_category")
    .removeClass("stop");
  elem.html(html);
}
...
function userpro_bm_addbookmark_category(elem, html) {
  elem
    .addClass("unbookmarked_category")
    .removeClass("bookmarked_category")
    .removeClass("stop");
  elem.html(html);
}
/*********Code Ended***************/
...
jQuery(document).ready(function () {
  ...
  jQuery(document).on("click", ".upb-action-remove i", function (e) {
    ...
    jQuery.ajax({
      url: userpro_ajax_url,
      data:
        "action=userpro_fav_removebookmark&post_id=" +
        post_id +
        "&collection_id=" +
        collection_id +
        "&category_id=" +
  ...
  jQuery(document).on("click", ".userpro-hard-remove", function (e) {
    ...
    jQuery.ajax({
      url: userpro_ajax_url,
      data:
        "action=userpro_hard_remove_collection&collection_id=" + collection_id,
      dataType: "JSON",
      type: "POST",
      success: function (data) {
  ...
  jQuery(document).on("click", ".userpro-soft-remove", function (e) {
    ...
    jQuery.ajax({
      url: userpro_ajax_url,
      data:
        "action=userpro_soft_remove_collection&collection_id=" + collection_id,
      dataType: "JSON",
      type: "POST",
      success: function (data) {
        list.find("a.active").remove();
  ...
  jQuery(document).on("click", ".userpro-coll-listpublic a", function (e) {
    ...
    container = jQuery(this)
      .parents(".userpro-coll")
      .find(".userpro-coll-body");
    if (container.hasClass("loading") == false) {
      ...
      container
        .addClass("loading")
        .find(".userpro-coll-body-inner")
        .find("div:not(.userpro-coll-remove)")
        .fadeTo(0, 0);

      jQuery.ajax({
        url: userpro_ajax_url,
        data:
          "action=userpro_change_public_collection&collection_id=" +
          collection_id +
          "&user_id=" +
          user_id,
        dataType: "JSON",
  ...
  jQuery(document).on("change", ".userpro-bm-list select", function (e) {
    ...
    bookmarked_category_link = dd
      .parents(".userpro-bm")
      .find("a.bookmarked_category");
    unbookmarked_category_link = dd
      .parents(".userpro-bm")
      .find("a.unbookmarked_category");
    unbookmarked_link = dd.parents(".userpro-bm").find("a.unbookmarked");
    post_id = jQuery(this).parents(".userpro-bm").data("post_id");
    collection_id = dd
      .parents(".userpro-bm")
      .find("input:hidden#collection_id")
      .val();
    jQuery.ajax({
      url: userpro_ajax_url,
      data:
        "action=userpro_fav_checkifbookmarked&collection_id=" +
        dd.val() +
        "&post_id=" +
        post_id,
      dataType: "JSON",
      ...
      success: function (data) {
        if (data.status) {
          userpro_bm_donebookmark(
            unbookmarked_link,
            dd.parents(".userpro-bm").data("remove_bookmark")
          );
          userpro_bm_donebookmark_category(
            unbookmarked_category_link,
            dd.parents(".userpro-bm").data("remove_bookmark_category")
          );
        } else {
          userpro_bm_addbookmark(
            bookmarked_link,
            dd.parents(".userpro-bm").data("add_to_collection")
          );
          userpro_bm_addbookmark_category(
            bookmarked_category_link,
  ...
  jQuery(document).on(
    "click",
    '.userpro-bm-dialog a[data-action="submit_collection"]',
    function (e) {
  ...
  jQuery(document).on(
    "submit",
    ".userpro-bm-dialog form:not(.stop)",
    function (e) {
      elem = jQuery(this);
      ...
      if (collection_name != "") {
        elem.addClass("stop");
        default_collection = jQuery(this)
          .parents(".userpro-bm")
        ...
        jQuery.ajax({
          url: userpro_ajax_url,
          data:
            "action=userpro_fav_addcollection&post_id=" +
            post_id +
            "&default_collection=" +
            default_collection +
            "&collection_name=" +
          ...
          success: function (data) {
            if (typeof data.errors == "undefined" && data.errors == null) {
              ...
              if (/*@cc_on!@*/ false) {
                jQuery(this)
                  .parents(".userpro-bm")
                  .find("#userpro_bm_collection")
                  .replaceWith(data.options);
              ...
              jQuery(this)
                ...
                .val(
                  jQuery(this)
                    .parents(".userpro-bm")
                    .find("#userpro_bm_collection option:last")
                    .val()
                )
              ...
              if (site_is_rtl) {
                jQuery(function () {
                  jQuery("select").attr(
                    "class",
                    jQuery("select").attr("class") + " chosen-rtl"
                  );
                  jQuery(".chosen-container-single").attr(
                    "class",
              ...
              jQuery(this)
                .parents(".userpro-bm")
                .find(".userpro-bm-list select")
                .trigger("change");
              userpro_bm_removedialog();
            } else {
              var cur_elem = elem
                .parents(".userpro-bm-act")
                .find(".userpro-bm-btn-contain.bm-right");
              userpro_bm_removedialog();
              userpro_bm_limitreached_dialog(
                cur_elem,
                "coll_limit_reached",
                "right",
                data.errors
  ...
  jQuery(document).on(
    "click",
    ".userpro-bm a[data-action=newcollection]",
    function (e) {
      elem = jQuery(this);
      parent = jQuery(this).parents(".userpro-bm");

      if (
        jQuery(this).parents(".userpro-bm").find(".userpro-bm-dialog form")
          .length == 0
      ) {
  ...
  jQuery(document).on(
    "click",
    ".userpro-bm a[data-action=bookmark].unbookmarked:not(.stop)",
    function (e) {
      elem = jQuery(this);
      parent = jQuery(this).parents(".userpro-bm");
      post_id = jQuery(this).parents(".userpro-bm").data("post_id");
      collection_id = jQuery(this)
        .parents(".userpro-bm")
        .find("#userpro_bm_collection")
        .val();
      remove_bookmark = jQuery(this)
        .parents(".userpro-bm")
        .data("remove_bookmark");
      dialog_bookmarked = jQuery(this)
        .parents(".userpro-bm")
        .data("dialog_bookmarked");
      ...
      jQuery.ajax({
        url: userpro_ajax_url,
        data:
          "action=userpro_fav_newbookmark&post_id=" +
          post_id +
          "&collection_id=" +
          collection_id,
        dataType: "JSON",
        ...
        success: function (data) {
          if (typeof data.errors == "undefined" && data.errors == null) {
            userpro_bm_update_active_collection(
              jQuery(this).parents(".userpro-bm"),
              data.collection_id
            );
            jQuery("#" + post_id)
              .removeClass("unbookmark")
              .addClass("addedbookmark");
  ...
  jQuery(document).on(
    "click",
    ".userpro-bm a[data-action=bookmark].bookmarked:not(.stop)",
    function (e) {
      elem = jQuery(this);
      parent = jQuery(this).parents(".userpro-bm");
      post_id = jQuery(this).parents(".userpro-bm").data("post_id");
      collection_id = jQuery(this)
        ...
        .find("#userpro_bm_collection")
        .val();
      add_to_collection = jQuery(this)
        .parents(".userpro-bm")
        .data("add_to_collection");
      dialog_unbookmarked = jQuery(this)
        .parents(".userpro-bm")
        .data("dialog_unbookmarked");
  ...
  jQuery(document).on(
    "click",
    ".userpro-bm a[data-action=bookmarkcategory].unbookmarked_category:not(.stop)",
    function (e) {
      elem = jQuery(this);
      parent = jQuery(this).parents(".userpro-bm");
      category_id = jQuery(this).parents(".userpro-bm").data("category_id");
      ...
      collection_id = jQuery(this)
        ...
        .find("#userpro_bm_collection")
        .val();
      remove_bookmark_category = jQuery(this)
        .parents(".userpro-bm")
        .data("remove_bookmark_category");
      dialog_bookmarked = jQuery(this)
        .parents(".userpro-bm")
        .data("dialog_bookmarked");
      post_id = jQuery(this).parents(".userpro-bm").data("post_id");
        ...
        for (i = 0; i < category_list.length - 1; i++) {
          if (jQuery(this).data("category") == category_list[i]) {
            jQuery.ajax({
              url: userpro_ajax_url,
              data:
                "action=userpro_fav_newcategorybookmark&category_id=" +
  ...
  jQuery(document).on(
    "click",
    ".userpro-bm a[data-action=bookmarkcategory].bookmarked_category:not(.stop)",
    function (e) {
      elem = jQuery(this);
      parent = jQuery(this).parents(".userpro-bm");
      category_id = jQuery(this).parents(".userpro-bm").data("category_id");
      collection_id = jQuery(this)
        ...
        .find("#userpro_bm_collection")
        .val();
      bookmark_category = jQuery(this)
        .parents(".userpro-bm")
        .data("bookmark_category");
      dialog_unbookmarked = jQuery(this)
        .parents(".userpro-bm")
        .data("dialog_unbookmarked");
      userpro_bm_newaction(elem, parent);
        ...
        for (i = 0; i < category_list.length - 1; i++) {
          if (jQuery(this).data("category") == category_list[i]) {
            jQuery.ajax({
              url: userpro_ajax_url,
              data:
                "action=userpro_fav_removecategorybookmark&category_id=" +
                jQuery(this).data("category") +
                "&collection_id=" +
                collection_id,
              dataType: "JSON",
...
function userpro_bookmark_icon(post_id, elm) {
  ...
  str =
    "action=userpro_bookmark_icon&post_id=" +
    post_id +
    "&condition=" +
    condition;
  jQuery.ajax({
