  ...
  $(function () {
    ...
    $(".trigger-ingredient-search").on("click", function () {
      ...
      if ($(".ingredients-multi-select").val() != null) {
        ...
        $btn
          .data("label-default", $btn.text())
          .text($btn.data("label-loading"));
        osetinGetIngredientsSearchResults();
      } else {
        $('.chosen-choices .search-field input[type="text"]').addClass(
          "animated animation-shake"
        );
      }
      return false;
    });

    $(".ingredients-search-results-w").on(
      "click",
      ".load-more-infinite",
      function () {
        var $btn = $(this);
    ...
    function osetinGetIngredientsSearchResults() {
      var paged = $(".load-more-infinite").length
        ? $(".load-more-infinite").data("paged")
        : 1;
      $.ajax({
        type: "POST",
        url: ajaxurl,
        ...
        success: function (data) {
          ...
          if ($(".load-more-infinite").length) {
            ...
            $load_more_btn
              .find("span")
              .text($load_more_btn.data("label-default"));
          }

          if (paged > 1) {
            $(".ingredients-search-results-w .masonry-grid").append(
              data.message
            );
          } else {
