(function ($) {
  ...
  $(function () {
    ...
    $(".gallery-image-next").on("click", function () {
      ...
      var $next_source = $item_media
        .find(".gallery-image-source.active")
        .next(".gallery-image-source");
      if (!$next_source.length)
        $next_source = $item_media.find(".gallery-image-source").first();
      $item_media.find(".gallery-image-source").removeClass("active");
      $next_source.addClass("active");
      $item_thumbnail.css(
        "background-image",
        "url(" + $next_source.data("gallery-image-url") + ")"
      );
    });
    ...
    $("#stop-timer-btn").click(function () {
      ...
      if ($(".timer-w").hasClass("is-counting")) {
        ...
        var countdown_minutes =
          "+" + Math.round($("#timer-minutes").val()) + "m";
    ...
    $(".share-recipe-btn, .trigger-share-recipe-lightbox, .share-meal-plan").on(
      "click",
      function () {
        if ($(".full-screen-share-box").length) {
          ...
          $("body").append(
            '<div class="full-screen-share-box"><div class="post-share-box">' +
              $(".post-share-box").html() +
              "</div></div>"
          );
        }
    ...
    if (
      $(".os-parallax").length &&
      not_display_type("phone") &&
      not_display_type("tablet")
    ) {
      ...
      var newBodyHeight = 0;
      var parallaxImageProportions =
        $osParallax.data("height") / $osParallax.data("width");
      var bodyProportions = bodyHeight / $("body").width();
      var imageCoveredHeight = Math.round(
        $("body").width() * parallaxImageProportions
      );

      var parallaxProportionsToBody = (
        (bodyHeight - windowHeight) /
        (imageCoveredHeight - windowHeight)
      ).toFixed(2);
      ...
      if (isSafari) {
        ...
        $(window).scroll(function () {
          ...
          if (newBodyHeight != bodyHeight) {
            imageCoveredHeight = Math.round(
              $("body").width() * parallaxImageProportions
            );
            parallaxProportionsToBody = (
              (newBodyHeight - windowHeight) /
              (imageCoveredHeight - windowHeight)
            ).toFixed(2);
          }
          var scroll_value = -Math.round(
            $(document).scrollTop() / parallaxProportionsToBody
          );
          $osParallax.css({
    ...
    if (
      not_display_type("phone") &&
      $(".single-panel-details-i").length &&
      $(".single-panel-details").hasClass("move-on-scroll")
    ) {
      // how to use this code
      $(window).scrolled(function () {
        if ($(".single-panel-details-i").data("current-offset")) {
          var currentOffset = $(".single-panel-details-i").data(
            "current-offset"
          );
        } else {
          var currentOffset = 0;
        ...
        var sidebarWrapperBottom =
          $(".single-panel-details").position().top +
          $(".single-panel-details").outerHeight();
        var sidebarBottom =
          $(".single-panel-details").position().top +
          $(".single-panel-details-i").outerHeight() +
          100;
        var sidebarTop =
          $(".single-panel-details").position().top + currentOffset;
        var panelDifference =
          $(".single-panel-details").height() -
          $(".single-panel-details-i").outerHeight();
        ...
        if ($(window).height() > $(".single-panel-details-i").height()) {
          emptyGap =
            $(window).height() - $(".single-panel-details-i").height() - 100;
        }
        ...
        if (screenBottom > sidebarBottom + currentOffset) {
          ...
          sidebar_offset =
            screenTop - $(".single-panel-details").position().top;
          if (sidebar_offset < 0) sidebar_offset = 0;
          $(".single-panel-details-i").css({
            "-webkit-transform": "translateY(" + sidebar_offset + "px)",
            "-moz-transform": "translateY(" + sidebar_offset + "px)",
    ...
    $(".ingredients-table tr .ingredient-action").on("click", function () {
      ...
      if ($tr.hasClass("ingredient-off")) {
        ...
        $tr
          .find("i.os-icon")
          .addClass("os-icon-circle-o")
          .removeClass("os-icon-check");
      } else {
        $tr.addClass("ingredient-off");
        $tr
          .find("i.os-icon")
          .addClass("os-icon-check")
          .removeClass("os-icon-circle-o");
      }
    });
    ...
    $("body").on("click", ".cooking-mode-toggler", function () {
      if ($(".cooking-mode-w").length) {
        ...
        $("body").append(
          '<div class="cooking-mode-w" font-change-count="0"><div class="cooking-mode-i">' +
            recipe_content +
            "</div></div>"
        );
    ...
    if ($post_gallery.length) {
      $post_gallery.owlCarousel({
        ...
        loop: false,
        nav:
          $post_gallery.find(".gallery-image-source").length > 4 ? true : false,
        dots: false,
        rtl: is_rtl,
        navText: [
          '<i class="os-icon os-icon-chevron-left"></i>',
          '<i class="os-icon os-icon-chevron-right"></i>',
        ],
      });
    }
    $(".single-post-gallery-images .gallery-image-source").on(
      "click",
      function () {
    ...
    $(".tooltip-trigger").on({
      mouseenter: function () {
        ...
        $(this).append(
          '<div class="tooltip-box"><div class="tooltip-box-header">' +
            tooltip_header +
            "</div></div>"
        );
      },
    ...
    var menu_timer;
    $(".menu-activated-on-hover > ul > li.menu-item-has-children").mouseenter(
      function () {
        var $elem = $(this);
    ...
    $(".menu-activated-on-hover > ul > li.menu-item-has-children").mouseleave(
      function () {
        var $elem = $(this);
    ...
    var sub_menu_timer;
    $(
      ".menu-activated-on-hover > ul > li.menu-item-has-children > ul > li.menu-item-has-children"
    ).mouseenter(function () {
      var $elem = $(this);
      ...
      $elem.addClass("active");
      if ($elem.length) {
        var sub_menu_right_offset =
          $elem.offset().left + $elem.outerWidth() * 2;
    ...
    $(".menu-activated-on-click li.menu-item-has-children > a").on(
      "click",
      function (event) {
