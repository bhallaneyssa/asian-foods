(function ($) {
  ...
  $(function () {
    ...
    var content_images = $(
      '.single-content-self img[class^="wp-image-"], .single-content-self img[class*=" wp-image-"]'
    );
    if (content_images.length) {
      content_images.each(function () {
        if (
          $(this).closest("a").length &&
          !$(this).closest("a").hasClass("osetin-lightbox-trigger-native")
        ) {
    ...
    $("body").on(
      "click",
      ".os-lb-navigation-next, .os-lb-active-image",
      function () {
        var current_index = $(".os-lb-thumbnails-w .active").index();
    ...
    $("body").on("click", ".os-lb-thumbnail-trigger", function () {
      ...
      $(".os-lb-active-image").css(
        "background-image",
        "url(" + lightbox_active_img_src + ")"
      );
      var lightbox_img_caption = $(this).data("lightbox-caption");
      if (
        typeof lightbox_img_caption !== "undefined" &&
        lightbox_img_caption != "" &&
        lightbox_img_caption != "undefined"
      ) {
      ...
      if (
        this_position_left >
          $(".os-lb-thumbnails-w").width() +
            $(".os-lb-thumbnails-w").scrollLeft() ||
        this_position_left < $(".os-lb-thumbnails-w").scrollLeft()
      ) {
    ...
    $("body").on(
      "click",
      ".osetin-lightbox-trigger, .gallery .gallery-item a, .osetin-lightbox-trigger-native, .osetin-lightbox-trigger-step-images",
      function () {
        $(".os-lightbox").remove();
        ...
        if ($(this).hasClass("osetin-lightbox-trigger")) {
          ...
          lightbox_img_caption = $(this).find("img").prop("alt");
          $(".gallery .gallery-item a, .osetin-lightbox-trigger-native").each(
            function () {
              thumbnails_captions_arr.push($(this).find("img").prop("alt"));
        ...
        for (i = 0; i < thumbnails_thumb_src_arr.length; i++) {
          if (lightbox_active_img_src == thumbnails_image_src_arr[i])
            active_thumbnails_class = "active";
          else active_thumbnails_class = "";
          thumbnails_html +=
            '<div class="os-lb-thumbnail-trigger ' +
            active_thumbnails_class +
            '" data-lightbox-caption="' +
            thumbnails_captions_arr[i] +
            '" data-image-src="' +
            thumbnails_image_src_arr[i] +
            '"><div class="thumbnail-item-bg" style="background-image:url(' +
            thumbnails_thumb_src_arr[i] +
            '); background-position: center center; background-repeat: no-repeat;"></div><div class="thumbnail-fader"></div></div>';
        }
        if (thumbnails_html != "") {
          thumbnails_html =
            '<div class="os-lb-thumbnails-w activate-perfect-scrollbar"><div class="os-lb-thumbnails-i">' +
            thumbnails_html +
            "</div></div>";
        }
        var close_btn_html =
          '<div class="os-lb-close-btn"><i class="os-icon os-icon-thin-close-round"></i></div>';
        if (thumbnails_thumb_src_arr.length > 1) {
          hide_thumbnails_btn_html =
            '<div class="os-lb-toggle-thumbnails-btn"><i class="os-icon os-icon-thin-hamburger"></i></div>';
          lightbox_navigation_btn_prev =
            '<div class="os-lb-navigation-link os-lb-navigation-prev"><i class="os-icon os-icon-thin-left-arrow-round"></i></div>';
          lightbox_navigation_btn_next =
            '<div class="os-lb-navigation-link os-lb-navigation-next"><i class="os-icon os-icon-thin-right-arrow-round"></i></div>';
        ...
        if (lightbox_img_caption) {
          var lightbox_img_caption_html =
            '<div class="lightbox-caption">' + lightbox_img_caption + "</div>";
        } else {
          var lightbox_img_caption_html =
            '<div class="lightbox-caption hidden"></div>';
        }
        var lightbox_active_image_html =
          '<div class="os-lb-active-image" style="background-image: url(' +
          lightbox_active_img_src +
          ')">' +
          lightbox_img_caption_html +
          "</div>";

        $(
          '<div class="os-lightbox has-thumbnails ' +
            hide_thumbnails_class +
            '">' +
            close_btn_html +
            lightbox_loader_html +
            lightbox_active_image_html +
            '<div class="os-lb-controls">' +
            lightbox_navigation_btn_prev +
            hide_thumbnails_btn_html +
            lightbox_navigation_btn_next +
            "</div>" +
        ...
        $thumbnails_i.width(
          110 * $thumbnails_i.find(".os-lb-thumbnail-trigger").length
        );

        $thumbnails_i.waitForImages().done(function () {
          var total_thumbnails_width = 0;
          $thumbnails_i
            .find(".os-lb-thumbnail-trigger .thumbnail-item-bg")
            .each(function () {
