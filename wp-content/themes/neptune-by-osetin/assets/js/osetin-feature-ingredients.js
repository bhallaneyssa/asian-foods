(function ($) {
  ...
  $(function () {
    ...
    function osetin_update_ingredient_amounts(
      $elements,
      initial_serves,
      new_serves
    ) {
      $elements.each(function () {
        ...
        var ingredient_initial_amount_text = String(
          $(this).data("initial-amount")
        );
        if (ingredient_initial_amount_text != "" && initial_serves > 0) {
          // extract only the numbers and dot before the first letter
          var amount_value =
            ingredient_initial_amount_text.match(/([0-9\.\/]+)[^0-9]*/);
          if (amount_value != null && typeof amount_value[1] !== "undefined") {
            ...
            var amount_non_value = ingredient_initial_amount_text.replace(
              amount_value[1],
              ""
            );
            // Fraction
    ...
    $(".ingredient-serves-incr").click(function () {
      ...
      var initial_serves = parseInt(
        $(".ingredient-serves-num").data("initial-service-num")
      );
      // check if current serves is proper number and not zero
      if (
        Number.isInteger(current_serves) &&
        current_serves > 0 &&
        initial_serves > 0 &&
        Number.isInteger(initial_serves)
      ) {
        var new_serves = current_serves + 1;
        // set input box values to new serving value
        $(".ingredient-serves-num")
          .val(new_serves)
          .data("current-serves-num", new_serves);
        // update ingredient amounts based on new servings
        osetin_update_ingredient_amounts(
          $(".ingredient-amount"),
          initial_serves,
          new_serves
        );
    ...
    $(".ingredient-serves-num").change(function () {
      var current_serves = parseInt(
        $(".ingredient-serves-num").data("current-serves-num")
      );
      var initial_serves = parseInt(
        $(".ingredient-serves-num").data("initial-service-num")
      );
